...
export function MarketingLoginPage() {
  ...
  useEffect(() => {
    if (!loading && user && userType === 'marketing') {
    ...
    <div className="min-h-screen flex items-center justify-center bg-surface/50 p-4">
      <div className="bg-white rounded-xl shadow-sm border border-navy-100 p-8 w-full max-w-sm text-center">
        <div className="mb-6">
          ...
          <p className="text-sm text-navy-400 mt-1">Marketing Portal</p>
        </div>
        ...
        <p className="text-xs text-navy-300 mt-6">
          Only authorized Leegality team members can access this portal.
        </p>
      </div>
    </div>
  );
